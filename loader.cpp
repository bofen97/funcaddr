...
open_bfd(std::string &fname)
{
   ...
   if(!bfd_check_format(bfd_h, bfd_object)) {
    ...
    if(bfd_get_flavour(bfd_h) == bfd_target_unknown_flavour) { fprintf(stderr, "unrecognized format for binary '%s' (%s)\n",
                 fname.c_str(), bfd_errmsg(bfd_get_error()));
       return NULL;
    }
...
load_binary_bfd(std::string &fname, Binary *bin, Binary::BinaryType type)
{
   ...
   switch(bfd_info->mach){
      case bfd_mach_arm_unknown:
         bin->arch = Binary::BinaryArch::ARCH_ARM64;
         bin->bits = 64;
         break;
      default:
         std::cout<<"unsupported arrchitecure "<< bin->arch_str <<std::endl;
         goto fail;
         break;
   }
